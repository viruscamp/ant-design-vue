...
| filteredValue | 当前筛选值，受控属性，外界可用此控制列的筛选状态，值为已筛选的 value 数组。在 template 中可用 `.sync` 后缀, 参见 `update:filteredValue` | string\[] | - |  |
...
| onFilteredValueChange <br> @filteredValueChange <br> @update:filteredValue | 当前筛选值`filteredValue`变化时调用，使用 template 或 jsx 时作为`filteredValueChange`或`update:filteredValue`事件使用 | function(filteredValue) {} | - |  |
